...
def estimate_size(video_info, resolution, remove_audio, fps, bitrate):
    ...
    estimated_bytes = (bitrate_value / 8) * duration * \
        resolution_factor * fps_factor * audio_factor
...
def main():
    ...
    st.write(
        "Compress your videos for easy sharing on Discord, WhatsApp, and other platforms.")

    # Check if FFmpeg is available
    if not FFMPEG_PATH or not FFPROBE_PATH:
        st.error(
            "⚠️ FFmpeg or FFprobe not found. Please ensure FFmpeg is properly installed on the server.")
        st.stop()
    ...
    resolution = st.sidebar.selectbox(
        "Resolution",
        options=["original", "1920x1080", "1280x720",
    ...
    if uploaded_files:
        ...
        with st.status("Processing uploaded videos...", expanded=True) as status:
            ...
            status.update(label="All videos processed successfully!",
            ...
            # At least 1, at most 3 columns
            num_cols = min(max(1, num_videos), 3)
            ...
            for row_idx, row in enumerate(rows):
                ...
                for i, (video_key, video_data) in enumerate(row):
                    with cols[i]:
                        st.write(
                            f"**{video_data['name']}** ({video_data['original_size']:.2f} MB)")
        ...
        if st.button("🎬 Compress Videos", use_container_width=True, type="primary"):
            ...
            selected_videos = [
                v for k, v in videos_data.items() if v["selected_for_compression"]]
            ...
            if not selected_videos:
                ...
                for i, video_data in enumerate(selected_videos):
                    ...
                    output_path = os.path.join(
                        tempfile.gettempdir(), output_filename)
                ...
                if compressed_videos:
                    ...
                    # Total statistics
                    total_original = sum(v["original_size"]
                                         for v in compressed_videos)
                    total_compressed = sum(v["compressed_size"]
                                           for v in compressed_videos)
                    ...
                    # At least 2, at most 4 columns
                    num_cols = min(max(2, num_videos), 4)
